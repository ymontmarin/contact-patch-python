...
class ExactConvexGeometricSimplex:
    ...
    def _compute_exact_geometric_insights(self):
        ...
        self.directions_from_pstar = np.array(
            [
                (self.A[:, i] - self.p_star)
                / (self.distances_to_pstar[i] + self.tolerance)
    ...
    def _compute_directional_alignment(self) -> np.ndarray:
        ...
        for i in range(self.n):
            if self.distances_to_pstar[i] > self.tolerance:
                ...
                alignment[i] = np.dot(
                    self.directions_from_pstar[i], self.c_direction_from_pstar
                )
    ...
    def _compute_radial_efficiency(self) -> np.ndarray:
        ...
        for i in range(self.n):
            distance_value = self.distances_to_pstar[
                i
            ]  # Proportional to s(p_i) = α ||p_i - p*||
    ...
    def _compute_optimal_direction_scores(self) -> np.ndarray:
        ...
        for i in range(self.n):
            ...
            pull_direction = (
                p_i - self.c
            )  # If we add weight here, expectation moves this way
    ...
    def _compute_ideal_pull_direction(self) -> np.ndarray:
        ...
        ideal_direction = ideal_direction / (
            np.linalg.norm(ideal_direction) + self.tolerance
        )
    ...
    def _enhanced_vertex_ordering(self) -> List[int]:
        ...
        for i in range(self.n):
            ...
            alignment_score = max(
                0, self.directional_alignment[i]
            )  # Alignment with c direction
    ...
    def _identify_exact_prunable_vertices(self) -> Set[int]:
        ...
        for i in range(self.n):
            ...
            bad_criteria_count = sum(
                [close_to_pstar[i], bad_direction[i], low_utility[i]]
            )
    ...
    def _smart_initialization_exact(self) -> Optional[Tuple[List[int], np.ndarray]]:
        ...
        print(
            f"   Farthest vertex from p*: {farthest_idx} (distance: {self.distances_to_pstar[farthest_idx]:.4f})"
        )
        ...
        candidates = [
            i
            for i in self.enhanced_vertex_priorities[: min(8, self.n)]
        ...
        for vertex_combo in itertools.combinations(candidates, 3):
            ...
            if solution is not None:
                ...
                if objective > best_objective:
                    ...
                    print(
                        f"   📈 New best: {vertex_combo} -> objective {objective:.6f}"
                    )

        if best_solution is not None:
            print(
                f"   ✅ Initial solution: vertices {best_solution[0]}, objective: {best_objective:.6f}"
            )
        else:
            print(
                "   ❌ No feasible initial solution found with heuristics, trying fallback..."
            )
    ...
    def _solve_3vertex_combination(
    ...
    def _enhanced_simplex_iteration(
        self, current_basis: List[int], current_weights: np.ndarray
        ...
        entering_candidates = [
            j for j in non_basic if reduced_costs[j] > self.tolerance
        ]
        ...
        if len(entering_candidates) > 1:
            ...
            print(
                f"   🔄 Multiple candidates, chose vertex {entering_var} (highest s-value: {self.S[entering_var]:.4f})"
            )
        else:
            entering_var = entering_candidates[0]
            print(
                f"   ➡️  Entering variable: {entering_var} (reduced cost: {reduced_costs[entering_var]:.6f})"
            )

        # Find leaving variable using ratio test
        leaving_var, new_weights = self._ratio_test(
            current_basis, current_weights, entering_var
        )
    ...
    def _ratio_test(
        self, basis: List[int], weights: np.ndarray, entering_var: int
        ...
        if new_solution is not None:
            ...
            for candidate_leaving in basis:
                if candidate_leaving != leaving_var:
                    test_basis = [
                        entering_var if j == candidate_leaving else j for j in basis
                    ]
    ...
    def verify_solution(self, solution: np.ndarray) -> bool:
        ...
        print(f"  c  = {self.c}")
        print(
            f"  Error: {constraint_error:.8f} {'✅' if constraint_error < 1e-6 else '❌'}"
        )
    ...
    def _recover_dual_with_geometric_regularization(
        self, optimal_basis: List[int]
        ...
        if np.linalg.norm(self.c - self.p_star) > self.tolerance:
            natural_gradient = (
                self.alpha
                * (self.c - self.p_star)
                / np.linalg.norm(self.c - self.p_star)
            )
        else:
    ...
    def _regularized_dual_2vertices(
        self, optimal_basis: List[int], natural_gradient: np.ndarray
    ) -> Tuple[np.ndarray, float]:
        """
        ...
        try:
            ...
            if np.linalg.det(A_T) > self.tolerance:
                ...
                if np.linalg.norm(direction) > self.tolerance:
                    lambda_star = np.dot(
                        base_mu - natural_gradient, direction
                    ) / np.dot(direction, direction)
                    mu_star = base_mu - lambda_star * direction

                    print(
                        f"   ✅ Regularized solution: μ* = {mu_star}, λ* = {lambda_star:.6f}"
                    )
                    print(
                        f"   📏 Distance from natural gradient: {np.linalg.norm(mu_star - natural_gradient):.6f}"
                    )
    ...
    def _regularized_least_squares(
        ...
        regularization_matrix = np.column_stack(
            [np.sqrt(gamma) * np.eye(2), np.zeros((2, 1))]
        )
...
def test_enhanced_simplex():
    ...
    S = np.array([alpha * np.linalg.norm(A[:, i] - p_star) for i in range(n)])
    ...
    if solution is not None:
        ...
        try:
            ...
            result = linprog(
                -S, A_eq=A_eq, b_eq=b_eq, bounds=[(0, None)] * n, method="highs"
            )
...
class PolygonPreprocessor:
    ...
    def _compute_geometric_structures(self):
        ...
        print(
            f"📐 Preprocessed {len(self.valid_triangles)} triangles, {len(self.valid_edges)} edges"
        )
...
class WarmStartManager:
    ...
    def find_best_warmstart(
        self, S_new: np.ndarray, p_star_new: np.ndarray, alpha_new: float
    ) -> Optional[dict]:
        """
...
class WarmStartConvexSimplex(ExactConvexGeometricSimplex):
    ...
    def solve_with_warmstart(
        self, S: np.ndarray, c: np.ndarray, p_star: np.ndarray, alpha: float
        ...
        mu_star, lambda_star = self.recover_dual_variables(
            current_basis, current_weights
        )
    ...
    def _smart_initialization_with_preprocessing(self) -> Tuple[List[int], np.ndarray]:
        ...
        for vertex_combo, combo_type in candidates:
            if combo_type == "triangle" and len(vertex_combo) == 3:
                ...
                if vertex_combo in self.preprocessor.basis_inverses:
                    weights = self.preprocessor.basis_inverses[
                        vertex_combo
                    ] @ np.hstack([self.c, 1.0])
        ...
        if best_solution:
            print(
                f"🚀 Preprocessing-based initialization: objective = {best_objective:.6f}"
            )
            return best_solution
        else:
