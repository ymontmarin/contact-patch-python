...
class PolygonContactPatch:
    def __init__(self, vis, mu, ker_precompute=False):
        ...
        """
        self.vis, _, _ = self.process_polygon(vis)
        self.mu = mu
        self.n = len(vis)
        assert self.n > 2
    ...
    @classmethod
    def generate_polygon(cls, N_sample=10, aimed_n=None, min_n=3, maxit=1000):
        for _ in range(maxit):
            points = np.random.randn((N_sample,2))
            ch = sc.spatial.ConvexHull(points)
            vis = points[ch.vertices]  # Counterclockwise sequence
            n = vis.shape[0]
            if n >= (min(aimed_n, min_n) if aimed_n is not None else min_n):
                if aimed_n is not None:
                    ...
                    query[-1] = 0
                    vis = vis[query]
                return cls.process_polygon(vis)
        raise Exception("N_sample must be increased")

    @staticmethod
    def test_polygon(vis):
        return np.all(np.isclose(np.sum(vis, axis=0), 0)) and np.all(np.isclose(np.sum(vis[:,0] * vis[:,1]), 0))

    @staticmethod
    def process_polygon(vis):
        # Center the vis:
        t = - vis.sum(axis=0, keepdims=True)
        vis += t
    ...
    def get_ATA(self):
        ...
        res.reshape(self.n, 3, self.n, 3)[:,:2,:,:2] = (
            np.einsum('ia,jb->i,a,j,b', self.wis, self.wis)
            + np.eye(2)[np.newaxis, :, np.newaxis, :]
        )
        # Add scalar part
        res.reshape(self.n, 3, self.n, 3)[:,2,:,2] =  np.einsum('ia,jb->i,j') + 1.
        return res
    ...
    def get_ATA_pinv(self):
        ...
        if we split in two diag element 3x3 diag(aat_inv**2) = [[D,0],[0,E]]

        We observe:
        pinv(ATA) = [[(B1TDB1 + E)  ...  (B1TDBn + E)]
                              ...
                     [(BnTDB1 + E)  ...  (BnTDBn + E)]]

        Again decomposing D as [[D',0],[0,d]]
        BiTBj = [[ wi(D'wj)T     0     ]
                 [   0         d wiTwj ]]
        """
        res = np.zeros((3 * self.n, 3 * self.n))
        ...
        # Add upper left part
        res.reshape(self.n, 3, self.n, 3)[:,:2,:,:2] = (
            np.einsum('ia,jb->i,a,j,b', self.wis, diag[:2][np.newaxis, :] * self.wis)
            + np.diag(diag[3:5])[np.newaxis, :, np.newaxis, :]
        )
        # Add scalar part
        res.reshape(self.n, 3, self.n, 3)[:,2,:,2] = diag[2] * np.einsum('ia,jb->i,j') + diag[5]
        return res
    ...
    def get_ATA_reg_inv(self, rho):
        ...
        And:
            (rI + ATA)^ = 1/r[I + A diag(-1/r(r + d)) AT]

        For A diag(-1/r(r + d)) AT it is exactly as in pinv(ATA) with another diag
        """
        res = np.zeros((3 * self.n, 3 * self.n))

        diag = 1. / ((self.aat + rho) * (-rho))
        ...
        res += np.diag(np.ones(3 * self.n) / rho)
        return res
    ...
    def is_inside_cone(self, l):
        """
        Check if l is in K
        Return True if sure in, False if sure out and None if unsure

        l in K <=> pinv(A)(l) + Ker(A) \cap K_mu^n \neq \emptyset

        so pinv(A)(l) \in K_mu^n => l in K

        pinv(A)(l) + Ker(A) \cap K_mu^n \neq 0 => for all i, p_i(pinv(A)(l)) + p_i(Ker(A)) \cap K_mu \neq \emptyset
        ...
        """
        fis = self.apply_pinvA(l)
        if self.is_inside_hidden_cone(fis):
            return True
        if self.ker_precompute and self.reject_nc_ker(fis):
            return False
    ...
    def reject_nc_ker(self, fis):
        ...
        The counter of a necessary condition based on A to be in the cone
        """
        for i, U in self.pkers.items():
            ...
            if rank == 1:
                # TODO
                # Check if affine line intersect cone
                continue
            if rank == 2:
                # TODO
                # Check if affine plane intersect cone
                continue
        return False
